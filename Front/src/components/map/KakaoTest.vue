        ...
        <div class="option">
          <div>
            <form>
              ...
              <input type="button" @click="searchPlaces" value="검색하기" />
              <input type="button" @click="searchAllList" value="전국" />
            </form>
          </div>
        </div>
        <hr />
...
export default {
  ...
  computed: {
    ...mapState(houseStore, ["houseallList"]),
  },
  watch: {
    houseallList(newv) {
      this.Allmarker(newv);
  ...
  methods: {
    ...mapActions(houseStore, ["getAllList"]),
    searchAllList() {
      this.getAllList();
    },
    Allmarker(newv) {
      ...
      for (var i = 0; i < newv.length; i++) {
        positions.push({ title: newv[i].apartmentName, latlng: new window.kakao.maps.LatLng(newv[i].lat, newv[i].lng) });
      }
